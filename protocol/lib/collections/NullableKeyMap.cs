...
public class NullableKeyMap<K, V> : IDictionary<K?, V>, IReadOnlyDictionary<K?, V>, IEquatable<NullableKeyMap<K, V>>
    where K : class
{
    ...
    public bool TryGetValue(K? key, [MaybeNullWhen(false)] out V value)
    {
        if (key == null)
        {
            value = _hasNullKey ? _nullKeyValue : default!;
            return _hasNullKey;
        }
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<K?, V>>
    {
        ...
        public void Reset()
        {
            ...
            _index = _index = _map._hasNullKey ? -2 : -1;
            _current = default;
        }
    ...
    public bool Equals(NullableKeyMap<K, V>? other)
    {
        if (other == null || Count != other.Count || !_comparer.Equals(other._comparer))
            return false;
        if (_hasNullKey != other._hasNullKey || _hasNullKey && !EqualityComparer<V>.Default.Equals(_nullKeyValue, other._nullKeyValue))
            return false;
        ...
        {
            if (pair.Key is null)
                continue; //Already checked null case
