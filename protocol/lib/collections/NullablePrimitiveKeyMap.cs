...
public class NullablePrimitiveKeyMap<K, V> : IDictionary<K?, V>, IReadOnlyDictionary<K?, V>
    where K : unmanaged
{
    ...
    public NullablePrimitiveKeyMap(IEnumerable<KeyValuePair<K?, V>> collection) : this(collection is ICollection<KeyValuePair<K?, V>> coll ? coll.Count : 0)
    {
    ...
    public bool ContainsKey(K? key)
    {
        ...
        {
            if (EqualityComparer<K>.Default.Equals(_entries![i].key, kv))
                return true;
    ...
    public bool Remove(K? key)
    {
        ...
        {
            if (!EqualityComparer<K>.Default.Equals(removeEntry.key, kv))
                return false;
    ...
    void ICollection<KeyValuePair<K?, V>>.CopyTo(KeyValuePair<K?, V>[] array, int index)
    {
        ...
        if (_hasNullKey)
            array[index++] = new KeyValuePair<K?, V>(null, _nullKeyValue);
