...
public class PrimitiveKeyMap<K, V> : IDictionary<K, V>, IReadOnlyDictionary<K, V>
    where K : unmanaged
{
    ...
    public PrimitiveKeyMap(IEnumerable<KeyValuePair<K, V>> collection) : this(collection is ICollection<KeyValuePair<K, V>> coll ? coll.Count : 0)
    {
    ...
    private bool TryInsert(K key, V value, bool overwrite)
    {
        ...
        int dstIndex;
        if (bucket == 0)
            dstIndex = _entries!.Length - 1 - _hiSize++; //Bucket is empty: Place new entry in the Hi-Region (non-colliding entries).
    ...
    public bool ContainsKey(K key)
    {
        ...
        {
            if (EqualityComparer<K>.Default.Equals(_entries![i].key, key))
                return true;
    ...
    public bool Remove(K key)
    {
        ...
        {
            if (!EqualityComparer<K>.Default.Equals(removeEntry.key, key))
                return false;
        ...
        else
        {
            ...
            if (EqualityComparer<K>.Default.Equals((removeEntry = ref _entries![removeIndex = (int)link]).key, key)) //Found the key in the *second* node of the chain.
                ...
                return false; //The second node is the terminal entry (in hi-region) but didn't match. So, the key is not in the map.
            else
                for (var collisions = 0; ;) //Continue traversing the lo-region chain.
                {
                    ...
                    if (_loSize <= removeIndex)
                        return false; //Reached Hi-Region (end of chain) without finding key.
                    if (_loSize + 1 < collisions++)
                        throw new InvalidOperationException("Concurrent operations not supported.");
                }
        }
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<K, V>>
    {
        ...
        public KeyValuePair<K, V> Current => _version != _map._version ? throw new InvalidOperationException("Collection was modified during enumeration.") : _index == -1 || _index == int.MaxValue - 1 ? throw new InvalidOperationException("Enumeration has either not started or has finished.")
                                                                                                                                                                                                         : _current;
