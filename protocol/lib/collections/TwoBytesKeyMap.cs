...
///</summary>
public class TwoBytesKeyMap<K, V> : IDictionary<K, V>, IReadOnlyDictionary<K, V>
    where K : unmanaged
{
    ...
    private bool TryInsert(K key, V value, bool overwrite)
    {
        while (true)
        {
            ...
            else if (_entries!.Length <= _loSize + _hiSize)
            {
                ...
                if (flatStrategyThreshold < newSize && _entries.Length < flatStrategyThreshold)
    ...
    public bool ContainsValue(V value)
    {
        if (IsFlatStrategy)
        {
            ...
            for (var i = 0; i < NULLS_SIZE; i++)
            {
                var bits = _nulls![i];
                if (bits == 0)
                    continue; //Skip empty 64-bit bits
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<K, V>>
    {
        ...
        public KeyValuePair<K, V> Current => _version != _map._version ? throw new InvalidOperationException("Collection was modified during enumeration.") : _index == -1 || (_map.IsFlatStrategy ? _index == 0x10000 : _index == _map._entries!.Length) ? throw new InvalidOperationException("Enumeration has either not started or has finished.")
                                                                                                                                                                                                                                                          : _current;
